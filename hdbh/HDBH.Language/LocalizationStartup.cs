...
namespace HDBH.Language
{
    internal class LocalizationStartup
    {
        ...
        private static string DefaultCulture
        {
            get
            {
                if (string.IsNullOrEmpty(defaultCulture))
                {
                    ...
                    defaultCulture = new CultureInfo("vi-VN").Name;
                }
        ...
        public string GetLang(string id, string _culture = "")
        {
            ...
            //_cached.Set(CacheModeEnum.Add, "getdsa", "abc", new TimeSpan(1, 0, 0, 0));

            //var cacheLanguage = HttpContext.Current.Session["sessionLang"];
            //if (cacheLanguage == null)
            //{
            //    string path = HttpContext.Current.Server.MapPath(string.Format("~/{0}/{1}.xml", xmlFolderName, Culture.Replace("-", "_")));
            ...
            //else
            //{
            //    List<LanguageModel> lsCache = (List<LanguageModel>)cacheLanguage;
            //    if (lsCache != null)
            //    {
            //        foreach (var item in lsCache)
            //        {
            //            if (item.Id.ToUpper().Trim().Equals(id.ToUpper().Trim()))
            //            {
            //                return item.Value;
            //            }
            //        }
